...
. Placer le fichier à tester dans le même répertoire que le fichier de test
. Importer le fichier à analyser dans le fichier de test 
. Remplacer les appels à RLE par nom_du_fichier.RLE(message, [nombre_iterations])
. Exécuter le fichier de test 

[facultatif] lorsque qu'on ne teste pas le temps ou la récursivité

=== Références

- Pour les tests : JUnit (Java), PyUnit (Python) 
